...
void template_matching_sablon(char *imag,pixel **imag_matrice,char *sablon,int nr_sablon,float prag, fereastra **D,int *n)
{
    ...
    S_med=medie_pixeli(sablon_matrice,inaltime_sablon,latime_sablon,0,0);    // intensitatea medie a pixelilor a sablonului
    S_dev=deviatie_standard(sablon_matrice,inaltime_sablon,latime_sablon,S_med,0,0); // deviatia standard a sablonului

    for(i=0;i<=inaltime_imag-inaltime_sablon;i++)
        for(j=0;j<latime_imag-latime_sablon;j++)
        {
            f_med=medie_pixeli(imag_matrice,inaltime_sablon,latime_sablon,i,j); // intensitatea medie a pixelilor pentru o portiune din matrice
            f_dev=deviatie_standard(imag_matrice,inaltime_sablon,latime_sablon,f_med,i,j); // deviatia standard a pixelilor pentru o portiune din matrice

            corr=corelatie(imag_matrice,sablon_matrice,inaltime_sablon,latime_sablon,i,j,f_med,S_med,f_dev,S_dev); // corelatia pentru coord (i,j)
...
int main()
{
    ...
    imagToMatrice(imagineI_gray,&imagineI_matrice_gray,inaltime,latime,pad); // transforma imaginea grayscale in matrice
    ...
    for(i=0;i<=9;i++) // pentru fiecare sablon are loc operatia de template matching
    {
       ...
       dimensiuni_imagine(cifra_sablon,&inaltime_sablon,&latime_sablon,&padding_sablon); // principalele caracteristici ale imaginii
    ...
    elim_non_maxime(D,&n,inaltime_sablon,latime_sablon); // functie care elimina din tabloul D cu n elemente non_maximele

    marcheaza_cifre(imagineI_matrice,inaltime_sablon,latime_sablon,D,n); // contureaza ferestrele ramase dupa eliminarea non-maximelor

    matriceToimagine(imagineI,"template_final.bmp",imagineI_matrice,inaltime,latime,pad); // salveaza in memoria externa noua imagine
